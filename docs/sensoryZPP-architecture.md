...
The **Sensory Zone of Partial Preservation (Sensory ZPP)** identifies the most caudal segment with preserved sensory function (light touch and pin prick) when sacral sparing is absent or not testable. It answers: *"What is the lowest level with any preserved sensation?"*
...
- If DAP is `No` or `NT` and either S4-5 LT or PP is *not* `canBeAbsentSensory` → `zpp = ['NA']`, `next = null` (stop).
...
export type StepDescription = { key: Translation; params?: { [key: string]: string } };
export type StepAction = { key: Translation; params?: { [key: string]: string } };
